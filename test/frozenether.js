...
function lengthen(account, index, duration, options) {
	var contract;
	var total;
	...
	return FrozenEther.deployed().then(function(instance) {
		...
		total = value.plus(duration);
		return contract.lenghtenFrozenState(index, duration, { from: account });
	}).then(function(tx) {
		if (options === undefined) {
			options = {};
		}
		options.amount = 0;
		return check(account, index, 0, total, options);
	});
}
...
contract('FrozenEther', function(accounts) {
	...
	it('Lengthen the duration of an acount', function() {
		return FrozenEther.deployed().then(function(instance) {
			return create(accounts[0], 1, 50, 1000);
		}).then(function() {
			return lengthen(accounts[0], 1, 500, { duration: 0 });
		}).then(function() {
			return increase_time(2000);
		}).then(function() {
			return lengthen(accounts[0], 1, 500, { duration: 0 });
		});
	});
